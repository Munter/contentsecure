...
function stringify(header, rules) {
    return header + ': ' + Object.keys(rules).map(function (key) {
            if (key === 'pageUrl' || rules[key].length === 0) {
                return '';
            }
...
module.exports = function (config) {
    ...
    return function (assetGraph, cb) {
        ...
        if (warnings.inlineScript.length) {
            console.warn(chalk.yellow('\n ! Warning: You are using inline JavaScript and haven\'t set "unsafe-inline"'));
            console.warn(chalk.yellow(' - ' + warnings.inlineScript.map(function (relation) {
                return relation.from.urlOrDescription;
            }).join('\n - ')));
        }

        if (warnings.inlineScript.length) {
            console.warn(chalk.yellow('\n ! Warning: You are using inline event handlers and haven\'t set "unsafe-inline"'));
            console.warn(chalk.yellow(' - ' + warnings.inlineScript.map(function (relation) {
                return relation.from.urlOrDescription;
            }).join('\n - ')));
        }
        ...
        if (warnings.inlineStyle.length) {
            console.warn(chalk.yellow('\n ! Warning: You are using inline style sheets and haven\'t set "unsafe-inline"'));
            console.warn(chalk.yellow(' - ' + warnings.inlineStyle.map(function (relation) {
                return relation.from.urlOrDescription;
            }).join('\n - ')));
        }

        if (warnings.inlineStyleAttribute.length) {
            console.warn(chalk.yellow('\n ! Warning: You are using inline style attributes and haven\'t set "unsafe-inline"'));
            console.warn(chalk.yellow(' - ' + warnings.inlineStyleAttribute.map(function (relation) {
                return relation.from.urlOrDescription;
            }).join('\n - ')));
        }
        ...
        allRules.forEach(function (rules) {
            console.log('\n' + chalk.green(rules.pageUrl + ' --> ') + stringify(headerName, rules));
        });
        ...
        console.log(chalk.green('\nSite --> ') + stringify(headerName, siteWide));
